...
async function temporaryDirectory(t) {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "mangani-"));
...
test("creates a complete offline starter", async (t) => {
  ...
  const projectPath = await createProject("kit00-dashboard", { cwd });
...
test("create command reports the generated project", async (t) => {
  ...
  assert.equal(
    await run(["create", "first-build"], capture.io, { cwd }),
    0
  );
...
test("rejects unsafe project names", () => {
  ...
  for (const name of unsafeNames) {
    assert.throws(() => validateProjectName(name), /Project name/);
  }
});
...
test("returns an error for unknown commands", async () => {
  ...
  assert.equal(await run(["deploy"], capture.io), 1);
